...
function parseArgs(args: string[]): RegisterArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--assistant-name': result.assistantName = args[++i] || 'StellarBot'; break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'StellarBot') {
    logger.info({ from: 'StellarBot', to: parsed.assistantName }, 'Updating assistant name');
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = content.replace(/^# StellarBot$/m, `# ${parsed.assistantName}`);
        content = content.replace(/You are StellarBot/g, `You are ${parsed.assistantName}`);
